...
export function App() {
  ...
  // Drag-and-drop (works on whole page)
    ...
    >
      ...
      <div style={topBarStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button style={buttonStyle} onClick={openFilePicker}>
            Open PLY
          </button>
        ...
        {stats && (
          <div style={{ display: 'flex', gap: 16, fontSize: 12, opacity: 0.8 }}>
            <span>{formatNumber(stats.numSplats)} splats</span>
            <span>{stats.fps} fps</span>
            {stats.loadTimeMs > 0 && <span>{Math.round(stats.loadTimeMs)} ms load</span>}
            {stats.gpuMemoryBytes > 0 && <span>{formatBytes(stats.gpuMemoryBytes)} GPU</span>}
          </div>
        )}
      </div>
      ...
      {loading && (
        <div style={overlayStyle}>
          ...
          <span style={{ marginTop: 16, fontSize: 14, opacity: 0.7 }}>Loading splats...</span>
        </div>
      )}
      ...
      {error && (
        <div style={overlayStyle}>
          ...
          <div style={{ marginTop: 8, fontSize: 13, maxWidth: 400, textAlign: 'center', opacity: 0.8 }}>
            {error}
          </div>
          <button
            style={{ ...buttonStyle, marginTop: 16 }}
            onClick={() => { setError(null); setSrc(null); }}
          >
            Back
          </button>
        </div>
      ...
      {dragging && (
        <div style={dragOverlayStyle}>
          <div style={{ fontSize: 20, fontWeight: 600 }}>Drop .ply file here</div>
        </div>
      )}
      ...
      {src && (
        <div style={hintStyle}>
          Left drag: rotate &middot; Right drag / Shift+drag: pan &middot; Scroll: zoom
        </div>
      )}
...
function WelcomeScreen({ onOpen }: { onOpen: () => void }) {
  return (
    <div style={welcomeStyle}>
      <div style={{ fontSize: 48, fontWeight: 800, letterSpacing: -2, marginBottom: 8 }}>
        ZSplat
      </div>
      <div style={{ fontSize: 14, opacity: 0.5, marginBottom: 32, maxWidth: 400, textAlign: 'center', lineHeight: 1.5 }}>
        WebGPU Gaussian Splat Renderer. Load any PLY file
        &mdash; standard or SuperSplat compressed &mdash; and
        explore millions of splats in real time.
      </div>
      <button style={welcomeButtonStyle} onClick={onOpen}>
        Open a .ply file
      </button>
...
const dragOverlayStyle: React.CSSProperties = {
  ...
  border: '3px dashed rgba(100,160,255,0.5)',
  zIndex: 30,
};
...
const spinnerStyle: React.CSSProperties = {
  ...
  borderTopColor: '#fff',
  borderRadius: '50%',
  animation: 'zsplat-spin 0.8s linear infinite',
};
